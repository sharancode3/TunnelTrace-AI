...
graph TB
    subgraph Untrusted_External_Boundary ["Untrusted External Boundary"]
        Client["Analyst Browser / PWA"]
        PCAP_In["Uploaded PCAP Files"]
    end

    subgraph DMZ___Ingress_Layer ["DMZ / Ingress Layer"]
        ReverseProxy["Nginx / Caddy Reverse Proxy"]
    end

    subgraph Execution_Class_A__Unprivileged ["Execution Class A (Unprivileged)"]
    ...
    subgraph Execution_Class_B__Privileged_Network_Agent ["Execution Class B (Privileged Network Agent)"]
...
graph TD
    subgraph Trust_Boundary_1__Internet___Public_Network ["Trust Boundary 1: Internet / Public Network"]
        Browser["User Browser"]
    end

    subgraph Trust_Boundary_2__Web_API_DMZ ["Trust Boundary 2: Web API DMZ"]
        API["FastAPI Service"]
    end

    subgraph Trust_Boundary_3__Internal_Processing__Class_A ["Trust Boundary 3: Internal Processing (Class A)"]
        Worker["Celery Worker"]
        TShark["TShark Subprocess"]
        DB[("PostgreSQL / Redis")]
    end

    subgraph Trust_Boundary_4__Privileged_OS__Class_B ["Trust Boundary 4: Privileged OS (Class B)"]
        Agent["Network Agent"]
...
graph LR
    subgraph Host_Root_Environment ["Host Root Environment"]
        HostNIC["Physical NIC: eth0"]
    end

    subgraph Isolated_Lab_Network_Namespace__ns_ipsec_lab ["Isolated Lab Network Namespace: ns-ipsec-lab"]
